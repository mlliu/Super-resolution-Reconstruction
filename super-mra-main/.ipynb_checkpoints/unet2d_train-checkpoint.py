...
combomatrix = [[32, 32, 8, 8, 9, 2000, 2000, 8, False]]
'''
...
reduction_list = [4] if testmode else [2, 3, 4, 5, 6]  # resolution reduction factors to train/predict
...
for iRed in reduction_list:  # loop over resolution reduction factors
    ...
    for b_index in combomatrix:  # loop over reconstructions to perform
        ...
        for optim in optimizers:  # loop through optimizers
            ...
            crop_train_y = 0.9 #0.50 if patches_from_volume else 0.6

            ###############################################################################
            # find distinct data sets
            ###############################################################################
            script_path = os.path.split(os.path.abspath(__file__))[0]
            dirtarget = os.path.join(script_path, "../../ARIC/scans/6_TOF_3D_5-slab_MIP_COR/NII/")
            #dirsource = "data/train/CentreSquare15p25Mask"
            dirsource = os.path.join(script_path, "../../ARIC/scans/low_resolution_cor/1DVarderMask"+reduction,"DICOM")
            ...
            if training_needed_flag:
                        ...
                        if patches_from_volume:  # select patches from each whole dataset
                            ...
                            if not unet_resnet_mode:
                                ...
                                0], ytrain_master_noaug[m * patches_per_set:(m + 1) * patches_per_set, :, :,
                                    0] = get_patches_within_volume_2D(vol_metric,
                                                                   [volume1[:, :],
                                                                    volume3[:, :]],
                ...
                print("indices_of_datasets_to_train is:",indices_of_datasets_to_train)
                # loop through "leave one out" iterations (i.e. distinct data sets)
                indices_of_datasets_to_train = [0]
                for set_to_train in indices_of_datasets_to_train:
                    print("set_to_train is:",set_to_train)
