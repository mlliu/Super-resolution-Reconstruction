...
fullySampledPath = r'../../../ARIC/scans/6_TOF_3D_5-slab_MIP_COR'#r'/run/media/soumick/Enterprise/Datasets/IXI/ISO_Resampled2T2/T1' #Root path containing fully sampled images (NIFTIs: .img, .nii, .nii.gz or DICOMs: .ima, .dcm)
...
underSampledOutPath =r'C:\\Users\\mliu121\\Desktop\\code\\ARIC\\scans\\low_resolution_cor'# r'/run/media/soumick/Enterprise/Datasets/IXI/ISO_Resampled2T2/BiLinear256/Under/T1' #Root path to store the undersampled output
outFolder = r'1DVarderMask50P2fold' #Inside the underSampledOutPath, this folder will be created. Inside which the undersampled results will be stored
zeropadOutput = False#True #By default set to True, when set to False doesn't zero pad the k-Space and decreases the pixel resolution of the output image. This should only be set True when using Cartesian CenterMasks
...
inputShape = (310,320)#(256,256) #This is a must have when not recalculating sampling patterns for each volume seperately. If recalculateUndersampling4Each set to True, then this is ignored. Also used when coil is not getting simulated for each
croporpad = False
interpolate = False
fullySampledCropPaddedPath = r''#r"/run/media/soumick/Enterprise/Datasets/IXI/ISO_Resampled2T2/BiLinear256/T1"#r'/run/media/soumick/Enterprise/Datasets/IXI/ISO_Resampled2T2/T1-BET-256'

undersamplingType = 0 #6 #Cartesian Samplings := 0: Varden1D, 1: Varden2D (updated version, old version is type 8 now), 2: Uniform, 3: CenterMaskPercent, 4: CenterMaskIgnoreLines, 5: CenterRatioMask, 6: CenterSquareMask, 7: High-frequency Mask, 8: Varden2Dv0 (Old version of Varden2D)
                      #Radial Samplings := 10: Golden Angle, 11: Equi-distance (Yet to be implimented)
percentOfKSpace = 0.01 #[between 0 and 1] Percent of k-Space to be sampled. To be used for Cartesian samplings except undersamplingType = 2, 4
...
def _undersample(fullImgVol, fullpath_file_under):
    #try:
        ...
        if recalculateUndersampling4Each:
            ...
            samplings = sampler.calculateSamplings(slice=fullImgVol, returnMeta=True)
            if(not isRadial):
                mask = samplings['mask'] 
...
for file in tqdm(files[1:]):
    ...
    #print("dicom_identifier",dicom_identifier)
...
for identifier, files in tqdm(dicoms.items()):
    #print("files",files)
    fullImgVol = ListRead(files,True).squeeze() #Squeeze to remove channel dim if only one channel
   
    fullImgVol = _croppad_interpolate(fullImgVol, inputShape, fullpath_file_fully) if croporpad or interpolate else fullImgVol
    #print("fullImgVol",fullImgVol.shape)
    if safeSliceUndersampling and fullImgVol.shape[-1] % sliceUndersamplingFactor != 0:
        print("Skipping as nSlice not divisable by slice undersampling factor")
        continue
